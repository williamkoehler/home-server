...
namespace server
{
    namespace networking
    {
        ...
        bool JsonApi::ProcessResApiCallHTTP(boost::beast::http::verb verb, std::string_view target,
                                            ...
                                            rapidjson::StringBuffer& output, std::string& contentType)
        {
            ...
            case boost::beast::http::verb::put: // Set
            {
                if (strncmp(target.data(), "user/photo", 10) == 0)
                {
                    ...
                    stbi_uc* raw =
                        stbi_load_from_memory((const stbi_uc*)input.data(), input.size(), &x, &y, &channels, 4);
                    if (raw == nullptr)
                        return false;
                    ...
                    const Ref<Resource> resource = resources->UpdateResourceFromMemoryWithoutCopy(
                        "user_photo", std::to_string(user->GetID()), image.data, image.size);
                    if (resource == nullptr)
                        return false;
