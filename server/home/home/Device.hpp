...
namespace home
{
	...
	public:
		...
		/// @return Event
		template<typename T>
		inline Ref<Event> AddEvent(const std::string& id, bool (T::* callback)(Ref<Device>)) { return AddEvent(id, (EventCallback<Device>*)callback); }
		...
		/// @return Timer
		template<typename T>
		inline Ref<Timer> AddTimer(const std::string& id, bool (T::* callback)(Ref<Device>)) { return AddTimer(id, (TimerCallback<Device>*)callback); }
	};
}