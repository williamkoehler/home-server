...
namespace server
{
	...
	public:
		...
		Ref<Resource> AddResourceFromMemoryWithoutCopy(std::string prefix, std::string name, uint8_t* data, size_t size);

		Ref<Resource> UpdateResourceFromMemory(std::string prefix, std::string name, uint8_t* data, size_t size);
		Ref<Resource> UpdateResourceFromMemoryWithoutCopy(std::string prefix, std::string name, uint8_t* data, size_t size);
