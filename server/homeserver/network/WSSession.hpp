...
namespace server
{
	...
	private:
		...
		void DoWSShutdown(boost::beast::websocket::close_code code = boost::beast::websocket::close_code::normal, const char* reason = "");
		void DoSSLShutdown(boost::system::error_code error);
		void OnShutdown(boost::system::error_code error);

	public:
		WSSession(Ref<ssl_socket_t> socket, Ref<User> user);
		~WSSession();
