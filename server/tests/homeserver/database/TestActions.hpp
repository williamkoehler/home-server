...
void UpdateUser(identifier_t id, const std::string& name, uint8_t* hash, uint8_t* salt, server::UserAccessLevel accessLevel)
{
	assert(id > 0);
...
void UpdateUserName(identifier_t id, const std::string& name)
{
	...
	//! Create user instance
	Ref<server::User> user = server::User::Create(userCache->name, userCache->id, sha256_empty, salt_empty, server::UserAccessLevel::kRestrictedUserAccessLevel);
	BOOST_REQUIRE_MESSAGE(user != nullptr, "Create server::User instance");
