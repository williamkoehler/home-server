...
BOOST_AUTO_TEST_CASE(test_update_room_prop)
{
	...
	UpdateRoomPropType(&room05, "this is a very looooooooong room type that is not very correct, but is only here to test if long strings are allowed");
	UpdateRoomPropType(&room02, "this is a very looooooooong room type that is not very correct, but is only here to test if long strings are allowed");
