...
void TestTable(sqlite3* database, const char* table, const Column* columns, size_t columnCount)
{
	...
	if (sqlite3_prepare_v2(database, R"(select name, type, "notnull", pk from pragma_table_info(?))", 65, &statement, nullptr) == SQLITE_OK)
	{
		...
		for (; sqlite3_step(statement) == SQLITE_ROW && index < columnCount; index++)
		{
			...
			//! Check column name
			BOOST_CHECK_MESSAGE(name == column.name, "(Table '" << table << "') Column name is not correct: '" << name << "' is not '" << column.name << "'");

			//! Check column type
			BOOST_CHECK_MESSAGE(type == column.type, "(Table '" << table << "') (Column '" << column.name << "' Type is not correct: '" << type << "' is not '" << column.type << "'");

			//! Check column not null flag
			if (column.notNull)
				BOOST_CHECK_MESSAGE(notNull == column.notNull, "(Table '" << table << "') (Column '" << column.name << "' NotNull flag is set");
			else
				BOOST_CHECK_MESSAGE(notNull == column.notNull, "(Table '" << table << "') (Column '" << column.name << "' NotNull flag is not set");

			//! Check column primary key flag
			if (column.primaryKey)
				BOOST_CHECK_MESSAGE(notNull == column.notNull, "(Table '" << table << "') (Column '" << column.name << "' Primary key flag is not set");
			else
				BOOST_CHECK_MESSAGE(notNull == column.notNull, "(Table '" << table << "') (Column '" << column.name << "' Primary key flag is set");
		}
