...
void UpdateUser(UserCache* userCache, const std::string& name, uint8_t* hash, uint8_t* salt,
                server::UserAccessLevel accessLevel)
{
    //! Update cache
    userCache->name = name;
    ...
    Ref<server::User> user =
        MockUser(userCache->id, userCache->name, userCache->hash, userCache->salt, userCache->accessLevel);
