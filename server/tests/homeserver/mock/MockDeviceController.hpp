...
public:
	MockDeviceControllerPlugin(identifier_t pluginID)
		: pluginID(pluginID)
	{ }

	virtual std::string GetPluginName() override { return "Mock Device Controller Plugin"; }
...
Ref<server::DeviceController> MockDeviceController(identifier_t id = 1, const std::string& name = "unknown devicecontroller", identifier_t pluginID = 1, Ref<server::Room> room = nullptr)
{
