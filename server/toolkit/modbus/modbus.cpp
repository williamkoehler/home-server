...
Modbus::Modbus() : host("127.0.0.1"), port(502), sock(0), msgID(0), slaveID(1)
{
}
Modbus::~Modbus()
{
...
bool Modbus::Connect()
{
    ...
    if (opt < 0 || fcntl(sock, F_SETFL, opt | O_NONBLOCK))
#endif
    {
        Close();
        return false;
...
const char* Modbus::GetException(uint8_t exception)
{
    static const char* exceptions[] = {
        "No error",           "Illegal function", "Illegal address",          "Illegal balue",
        "Server failure",     "Acknowledge",      "Server or slave busy",     "Negative acknowledge",
        "Memory party error", "No connection",    "Gateway path unavailable", "Gateway target device failed to respond",
    };
    if (exception < 12)
    ...
    do
    {
        ...
        if (leftBytes == 0)
        {
            ...
            case ReceiveState::ReceiveHeader: {
                ...
                // Verify length
                leftBytes =
                    static_cast<ssize_t>(static_cast<uint16_t>(msg[4]) << 8 | static_cast<uint16_t>(msg[5])) - 2;
                if (leftBytes <= 0)
                    return -1;
